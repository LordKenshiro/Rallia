  ...
  for (const facility of createdFacilities) {
    try {
      ...
      const { error: contactsError } = await supabase
        ...
        .eq("facility_id", facility.id);
      if (contactsError)
        rollbackErrors.push(`facility_contacts: ${contactsError.message}`);
      ...
      if (facilityImages && facilityImages.length > 0) {
        ...
        if (imagesDeleteError)
          rollbackErrors.push(`facility_images: ${imagesDeleteError.message}`);
      }
    } catch (error: any) {
      rollbackErrors.push(
        `Facility rollback error: ${
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (orgError) {
      console.error(
        "[Admin Org Create] Organization creation error:",
        orgError
      );
      return NextResponse.json(
    ...
    try {
      ...
      for (
        ...
        if (!facilityData.name || facilityData.selectedSports.length === 0) {
          throw new Error(
            `Facility ${facilityIndex + 1} is missing required fields`
          );
        }
        ...
        if (facilityError) {
          throw new Error(
            `Failed to create facility: ${facilityError.message}`
          );
        }
        ...
        // Upload images
        const imageCount =
          (facilityData as any).imageCount || facilityData.images?.length || 0;
        if (imageCount > 0) {
        ...
        for (const courtRow of facilityData.courtRows) {
          ...
          if (courts.length > 0) {
            ...
            if (courtsError) {
              throw new Error(
                `Failed to create courts: ${courtsError.message}`
              );
            }
      ...
      const rollbackErrors = await rollbackFacilities(
        supabase,
        createdFacilities,
        createdOrg
      );
