...
export default function FacilityAvailabilityPage() {
  ...
  const fetchSchedule = useCallback(async () => {
    ...
    // Fetch existing facility-level templates (where court_id is NULL)
  ...
  // When slot duration changes, optionally adjust all slots
  const handleSlotDurationChange = (newDuration: number) => {
    setSlotDuration(newDuration);
    // Optionally auto-adjust all slots - for now just update duration
    // Users will see validation warnings if times are incompatible
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Delete all existing facility-level templates
      await supabase.from('court_slot').delete().eq('facility_id', facilityId).is('court_id', null);
    ...
    <div className="flex flex-col w-full gap-8 max-w-4xl mx-auto">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            {t('facilityTemplate.title')}
          </CardTitle>
          <CardDescription>{t('facilityTemplate.description')}</CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          {/* Slot Duration & Default Price */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              <label className="text-sm font-medium">{t('facilityTemplate.slotDuration')}</label>
              <Select
                value={slotDuration.toString()}
                onValueChange={(v: string) => handleSlotDurationChange(parseInt(v))}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
              ...
              <p className="text-xs text-muted-foreground">
                {t('facilityTemplate.slotDurationHint')}
              </p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                {t('facilityTemplate.defaultPriceHint')}
              </p>
            </div>
          </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>{t('facilityTemplate.subtitle')}</CardDescription>
        </CardHeader>
                          ...
                          <div className="flex items-center gap-3 flex-wrap">
                            <div className="flex items-center gap-2">
                              ...
                              <Input
                                ...
                                className={cn(
                                  'w-32',
                                  slotErrors.some(e => e.field === 'start_time') && 'border-red-500'
                                )}
                              />
                            </div>
                            ...
                            {hasSlotIssues && slotWarnings.some(w => w.field === 'duration') && (
                              <TooltipProvider>
                                <Tooltip>
                                  <TooltipTrigger asChild>
                                    <Button
      ...
      <div className="flex gap-4">
        ...
        <Button variant="outline" asChild>
          <Link href={`/dashboard/facilities/${facilityId}`}>Cancel</Link>
        </Button>
      </div>
    </div>
  );
