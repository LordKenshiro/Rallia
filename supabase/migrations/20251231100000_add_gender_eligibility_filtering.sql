-- Migration: Add gender eligibility filtering to match search RPCs
-- Description: Updates search_public_matches and search_matches_nearby to:
--   1. Filter matches based on the viewing user's gender (p_user_gender)
--   2. Only show matches where preferred_opponent_gender is NULL (open to all)
--      or matches the user's gender
--   3. Keep the UI filter (p_gender) for additional narrowing within eligible matches
--   4. Fix search_matches_nearby to use cancelled_at IS NULL instead of status = 'scheduled'

-- =============================================================================
-- STEP 1: Update search_public_matches with gender eligibility filtering
-- =============================================================================
...
BEGIN
  ...
  WHERE m.visibility = 'public'
    ...
    -- Match type filter (casual/competitive/both)
    AND (
      p_match_type IS NULL 
      OR (p_match_type = 'casual' AND m.match_type IN ('casual', 'both'))
