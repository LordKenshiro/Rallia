-- ============================================================================
-- Migration: Fix search_public_matches custom location detection
-- Created: 2026-01-20
-- Description: Fix the CASE statement in search_public_matches to use
--              location_type instead of facility_id for determining how to
--              calculate distance. This ensures custom location matches with
--              proper coordinates are correctly included and have their
--              distances calculated from custom_latitude/custom_longitude.
--
-- The issue: The CASE statement was checking m.facility_id IS NOT NULL first,
-- which could incorrectly match custom location matches that might have
-- leftover facility_id values. The correct approach (used by search_matches_nearby)
-- is to check m.location_type = 'facility' explicitly.
-- ============================================================================
...
-- =============================================================================
-- STEP 2: Recreate with fixed location type detection
-- =============================================================================
  ...
  WITH match_distances AS (
    ...
    SELECT 
      ...
      CASE
        ...
        -- FIXED: Check location_type instead of facility_id to correctly handle custom locations
...
COMMENT ON FUNCTION search_public_matches IS 'Search public matches with comprehensive filters. Uses location_type (not facility_id) to correctly detect custom location matches. Enum columns are cast to TEXT for comparison with text parameters.';
