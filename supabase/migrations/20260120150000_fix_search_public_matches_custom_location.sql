-- ============================================================================
-- Migration: Fix search_public_matches to ensure custom location matches are included
-- Created: 2026-01-20
-- Description: Fix multiple issues preventing custom location matches from appearing:
--
--   1. NULL comparison bug: `p_date_range != 'today'` returns NULL when p_date_range
--      is NULL, which fails the WHERE clause. Fixed by using COALESCE.
--
--   2. Time comparison issue: LOCALTIME AT TIME ZONE returns TIMETZ, but match.start_time
--      is TIME without timezone. Fixed by casting (NOW() AT TIME ZONE tz)::TIME.
--
--   3. Explicit custom location inclusion: Added explicit checks in match_distances CTE
--      to ensure custom location matches with valid coordinates are included.
-- ============================================================================
...
-- =============================================================================
-- STEP 2: Recreate with explicit custom location handling
-- =============================================================================
...
BEGIN
  ...
  WITH match_distances AS (
    ...
    -- IMPORTANT: Explicitly handle custom location matches to ensure they're included
    SELECT 
      m.id,
      CASE
  ...
  filtered_matches AS (
    ...
    WHERE
      ...
      -- FIXED: Use COALESCE to handle NULL p_date_range (NULL != 'today' returns NULL, not TRUE)
      AND (
        COALESCE(p_date_range, 'all') != 'today' 
        OR m.match_date > v_date_start 
        OR (
          m.match_date = v_date_start 
          AND m.start_time > (NOW() AT TIME ZONE COALESCE(m.timezone, 'UTC'))::TIME
        )
      )
...
COMMENT ON FUNCTION search_public_matches IS 'Search public matches with comprehensive filters. Fixes NULL comparison bug in date_range filter and properly handles custom location matches. Uses location_type to correctly detect custom location matches. Enum columns are cast to TEXT for comparison with text parameters.';
