-- ============================================================================
-- Revert Edge Function triggers to use x-service-key custom header
-- ============================================================================
-- This migration reverts the database triggers that call Edge Functions back to
-- using the custom x-service-key header instead of Authorization: Bearer.
-- The Bearer header gets stripped by some proxies/gateways, causing auth failures.
-- ============================================================================
...
BEGIN
  ...
  -- Call send-email Edge Function with x-service-key custom header
...
COMMENT ON FUNCTION public.notify_new_invitation() IS 
  'Calls send-email Edge Function when a new invitation is created. Uses x-service-key custom header for authentication.';
...
BEGIN
  ...
  -- Call the Edge Function using pg_net with x-service-key custom header
  -- Note: We use a custom header because Authorization: Bearer gets stripped by some proxies
  SELECT INTO request_id net.http_post(
...
COMMENT ON FUNCTION public.notify_send_notification() IS 
  'Trigger function that calls the send-notification Edge Function when a notification is inserted. Uses x-service-key custom header for authentication.';
...
DO $$
BEGIN
  RAISE NOTICE 'Reverted all Edge Function triggers to use x-service-key custom header authentication';
END $$;
