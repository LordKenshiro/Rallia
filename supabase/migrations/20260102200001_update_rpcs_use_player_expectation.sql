-- ============================================================================
-- Migration: Update RPCs to use player_expectation instead of match_type
-- Created: 2026-01-02
-- Description: After removing the duplicate match_type column, update all RPC
--              functions to use player_expectation instead.
-- ============================================================================
  ...
  match_counts AS (
    SELECT 
      ...
      CASE fm.format 
        ...
        ELSE 2 
      END AS total_spots,
      -- Count only joined participants (not the creator)
      (
        SELECT COUNT(*) 
        FROM match_participant mp 
        WHERE mp.match_id = fm.id AND mp.status = 'joined'
      ) + 1 AS filled_spots  -- +1 for creator
...
-- =============================================================================
-- STEP 2: Update search_matches_nearby RPC
-- =============================================================================
