-- Migration: Update search_public_matches to support all location types
-- Description: When no distance filter is applied, fetch ALL location types (facility, custom, tbd).
--              When a distance filter IS applied, only fetch facility and custom location matches
--              within that distance (using facility.location for facility matches and
--              match.custom_latitude/custom_longitude for custom location matches).
...
COMMENT ON FUNCTION search_public_matches IS 'Search public matches with comprehensive filters. When p_max_distance_km is NULL, returns matches of ALL location types (facility, custom, tbd). When p_max_distance_km has a value, only returns facility and custom location matches within that distance. Supports: text search, format, match type, date range, time of day, skill level, gender preference, cost, and join mode filters.';
