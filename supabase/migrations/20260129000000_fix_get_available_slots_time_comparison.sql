-- ============================================
-- FIX: Improve time comparison in get_available_slots
-- ============================================
-- The OVERLAPS operator might have edge cases with time boundaries.
-- This migration uses explicit time comparisons to ensure booked slots
...
BEGIN
    ...
    -- Use explicit time comparisons instead of OVERLAPS for better reliability
    booked_times AS (
    ...
    -- Exclude booked slots - use explicit comparison
    -- A slot overlaps with a booking if: slot_start < booking_end AND slot_end > booking_start
    WHERE NOT EXISTS (
        SELECT 1 FROM booked_times bt
        WHERE gs.slot_start < bt.end_time AND gs.slot_end > bt.start_time
    )
    -- Exclude blocked slots
    AND NOT EXISTS (
        SELECT 1 FROM blocked_times bl
        WHERE bl.start_time IS NULL  -- entire day block
           OR (gs.slot_start < bl.end_time AND gs.slot_end > bl.start_time)
    )
...
COMMENT ON FUNCTION get_available_slots(UUID, DATE) IS 
    'Returns available booking slots for a court on a specific date.
     ...
     Uses explicit time comparisons to exclude booked and blocked slots.
