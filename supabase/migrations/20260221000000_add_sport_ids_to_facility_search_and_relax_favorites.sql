-- Migration: Add sport_ids to facility search results and relax favorites constraint
-- Description:
--   1. Modifies search_facilities_nearby to return sport_ids UUID[] (the subset of
--      p_sport_ids each facility supports). Replaces DISTINCT ON with GROUP BY.
--   2. Relaxes player_favorite_facility display_order constraint from <= 3 to <= 6
--      so dual-sport users can store up to 6 favorites.
...
-- Recreate with sport_ids in the return type
...
-- Relax display_order constraint from <= 3 to <= 6 for dual-sport users
ALTER TABLE player_favorite_facility DROP CONSTRAINT player_favorite_facility_order_check;
ALTER TABLE player_favorite_facility ADD CONSTRAINT player_favorite_facility_order_check
  CHECK (display_order >= 1 AND display_order <= 6);

COMMENT ON COLUMN player_favorite_facility.display_order IS 'Order of display (1-6), allows user to prioritize favorites';
