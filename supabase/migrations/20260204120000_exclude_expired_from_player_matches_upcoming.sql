-- Migration: Exclude expired matches from get_player_matches upcoming
-- Description: For timeFilter = 'upcoming', exclude matches that are expired:
--              start_time has passed AND match is not full (joined count < capacity).
--              Such matches should not appear in My Matches (Home, PlayerMatches).
...
COMMENT ON FUNCTION get_player_matches(UUID, TEXT, UUID, INT, INT, TEXT) IS 'Get matches for a player with optional status filtering. Upcoming excludes expired matches (start passed, not full). Supports filters: all, hosting, confirmed, pending, requested, waitlisted, needs_players, ready_to_play (upcoming), feedback_needed, played, hosted, as_participant, expired, cancelled (past).';
