...
function main() {
  ...
  if (CREATE_TEMPLATE) {
    ...
    log('\n  ✓ Templates created! Copy them to actual .env files and fill in values.', 'green');
    process.exit(0);
  }
  ...
  if (totalIssues === 0 && totalWarnings === 0) {
    log('  ✓ All environments properly configured! 🎉', 'green');
    process.exit(0);
  } else {
    if (totalIssues > 0) {
      log(`  ✗ ${totalIssues} missing required variable(s)`, 'red');
    }
    if (totalWarnings > 0) {
    ...
    log('\n  💡 Quick fixes:', 'cyan');
    log('     • Run: npm run check-env -- --create-template', 'dim');
    log('     • Copy .env.example files to .env and fill in values', 'dim');
    log('     • Ask a team member for the required values', 'dim');
